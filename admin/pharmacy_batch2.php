				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								...
								<input type="text" name="pharmacy_batch2_name" class="form-control" required>
							</div>
                            <div class="form-group">
								<label class="control-label">Registration No:</label>
								<input type="text" name="pharmacy_batch2_reg" class="form-control" required>
							</div>
                            <div class="form-group">
								<label class="control-label">Current Designation</label>
								<input type="text" name="pharmacy_batch2_desgination" class="form-control" required>
							</div>
					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
                            ...
                            <?php
                                ...
                                $pharmacy_batch2_id = "";
                                $pharmacy_batch2_name = "";
                                $pharmacy_batch2_reg ="";
                                $pharmacy_batch2_desgination ="";
                                $query = "select * from pharmacy_batch2 ";
                                $query_run = mysqli_query($connection,$query);
                                while($row = mysqli_fetch_assoc($query_run)){
                                $pharmacy_batch2_id = $row['pharmacy_batch2_id'];
                                $pharmacy_batch2_name = $row['pharmacy_batch2_name'];
                                $pharmacy_batch2_reg = $row['pharmacy_batch2_reg'];
                                $pharmacy_batch2_desgination = $row['pharmacy_batch2_desgination'];

                             
                                ?>
								<tr>
									<td class="text-center">
                                    <?php echo $pharmacy_batch2_id ?>
                                    </td>
									<td class="">
										<?php echo $pharmacy_batch2_name?>
									</td>
                                    <td class="">
										<?php echo $pharmacy_batch2_reg?>
									</td>
                                    <td class="">
										<?php echo $pharmacy_batch2_desgination?>
									</td>
...
<script>
  ...
  <?php
	if(isset($_POST['add'])){
        ...
        $query = "insert into pharmacy_batch2 values(null,'$_POST[pharmacy_batch2_name]','$_POST[pharmacy_batch2_reg]','$_POST[pharmacy_batch2_desgination]')";
        $query_run = mysqli_query($connection,$query);
        echo "<script>location.href='pharmacy_batch2.php'</script>";
        }
  ?>
