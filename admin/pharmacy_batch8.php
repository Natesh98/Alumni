				...
				<div class="card">
					...
					<div class="card-body">
							...
							<div class="form-group">
								...
								<input type="text" name="pharmacy_batch8_name" class="form-control" required>
							</div>
                            <div class="form-group">
								<label class="control-label">Registration No:</label>
								<input type="text" name="pharmacy_batch8_reg" class="form-control" required>
							</div>
                            <div class="form-group">
								<label class="control-label">Current Designation</label>
								<input type="text" name="pharmacy_batch8_desgination" class="form-control" required>
							</div>
					...
					<div class="card-body">
						<table class="table table-bordered table-hover">
                            ...
                            <?php
                                ...
                                $pharmacy_batch8_id = "";
                                $pharmacy_batch8_name = "";
                                $pharmacy_batch8_reg ="";
                                $pharmacy_batch8_desgination ="";
                                $query = "select * from pharmacy_batch8 ";
                                $query_run = mysqli_query($connection,$query);
                                while($row = mysqli_fetch_assoc($query_run)){
                                $pharmacy_batch8_id = $row['pharmacy_batch8_id'];
                                $pharmacy_batch8_name = $row['pharmacy_batch8_name'];
                                $pharmacy_batch8_reg = $row['pharmacy_batch8_reg'];
                                $pharmacy_batch8_desgination = $row['pharmacy_batch8_desgination'];

                             
                                ?>
								<tr>
									<td class="text-center">
                                    <?php echo $pharmacy_batch8_id ?>
                                    </td>
									<td class="">
										<?php echo $pharmacy_batch8_name?>
									</td>
                                    <td class="">
										<?php echo $pharmacy_batch8_reg?>
									</td>
                                    <td class="">
										<?php echo $pharmacy_batch8_desgination?>
									</td>
...
<script>
  ...
  <?php
	if(isset($_POST['add'])){
        ...
        $query = "insert into pharmacy_batch8 values(null,'$_POST[pharmacy_batch8_name]','$_POST[pharmacy_batch8_reg]','$_POST[pharmacy_batch8_desgination]')";
        $query_run = mysqli_query($connection,$query);
        echo "<script>location.href='pharmacy_batch8.php'</script>";
        }
  ?>
